...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const grCarousel = document.getElementById('gr-carousel');
    if (grCarousel) {
        const reviews = [
            { name: "Anton Hämelin", date: "2 months ago", text: "Nya salen är toppen, med stor träningssal och nya omklädningsrum. Gemenskapen är otrolig och alla känner sig välkomna oberoende ursprung. Nivån på de som tränar är hög. Rekommenderar starkt 🥋" },
            { name: "Lauri Huusko", date: "2 months ago", text: "Erinomainen BJJ-sali! Hyvä ja kannustava treeni-ilmapiiri, osaavat valmentajat ja todella mukava porukka. Sopii hyvin sekä aloittelijoille että kokeneemmille treenaajille. Voin lämpimästi suositella! 🥋" },
            { name: "Henrik Setterberg", date: "2 months ago", text: "Bra klubb i nya lokaler för dig som vill lära dig BJJ." },
            { name: "Nicklas Lindholm", date: "2 months ago", text: "" },
            { name: "Sergiy Artuh", date: "2 months ago", text: "Great place to train for all levels (also regular basic courses for complete beginners). Sessions are held in English but coaches speak Finnish and Swedish when needed. Friendly atmosphere, you can choose busy days or not busy days if you like" },
            { name: "Jonas Kraufvelin", date: "2 months ago", text: "Huolet ja stressi jäävät kun astut matolle! Treeneissä on aina hyvä fiilis ja mahtava porukka! 💪🏼" },
            { name: "Erik Wikström", date: "2 months ago", text: "" },
            { name: "Benjamin Björkqvist", date: "2 months ago", text: "Really good training, skilled coaches, and great people 👍" },
            { name: "Jacob Granqvist", date: "2 months ago", text: "Best team ever! Great atmosphere and highest standard of every class. Same goes for both the standard and junior sessions! Go Gorilla! 🤘 🦍 💪" },
            { name: "Jaana Alanen", date: "2 months ago", text: "Paras sali missä olen ollut ❤️ Mukavia ihmisiä, kovatasoisia sparreja ja huipputason valmentajat ja seminaarit!" },
            { name: "riku vuorisalo", date: "2 months ago", text: "Really nice place, and people." }
        ];
        ...
        const getAvatar = (name) => {
            ...
            return `https://ui-avatars.com/api/?name=${initials}&background=${color}&color=fff&size=128&font-size=0.4`;
        };

        const starsHtml = `<svg viewBox="0 0 24 24" width="16" height="16" fill="#FABB05"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>`.repeat(5);

        grCarousel.innerHTML = reviews.map(r => `
            <div class="gr-card">
                <div class="gr-card__header">
                    <div class="gr-card__avatar">
                        <img src="${getAvatar(r.name)}" alt="${r.name}" loading="lazy" width="40" height="40">
                        <div class="gr-card__g-badge">
                            <svg viewBox="0 0 24 24" width="12" height="12" xmlns="http://www.w3.org/2000/svg"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/><path d="M1 1h22v22H1z" fill="none"/></svg>
                        </div>
                    </div>
                    <div class="gr-card__meta">
                        <span class="gr-card__name">${r.name}</span>
                        <span class="gr-card__date">${r.date}</span>
                    </div>
                </div>
        ...
        // Navigation arrows
        const prevBtn = document.querySelector('.gr-widget__nav--prev');
        const nextBtn = document.querySelector('.gr-widget__nav--next');
        const scrollAmount = window.innerWidth > 1024 ? 400 : 320; // Approx width to scroll

        if (prevBtn && nextBtn) {
            prevBtn.addEventListener('click', () => grCarousel.scrollBy({ left: -scrollAmount, behavior: 'smooth' }));
            nextBtn.addEventListener('click', () => grCarousel.scrollBy({ left: scrollAmount, behavior: 'smooth' }));
            
            // Optional: Hide/Show arrows on edges
            grCarousel.addEventListener('scroll', () => {
                prevBtn.style.opacity = grCarousel.scrollLeft <= 0 ? '0.3' : '1';
                nextBtn.style.opacity = grCarousel.scrollLeft >= (grCarousel.scrollWidth - grCarousel.clientWidth - 10) ? '0.3' : '1';
            }, { passive: true });
        }
    }
    ...
    (function initGoldenOrb() {
        ...
        const frag = `
            ...
            vec4 mainImage(vec2 fragCoord) {
                ...
                uv.x += hover * hoverIntensity * 0.05 * sin(uv.y * 6.0 + t);
                uv.y += hover * hoverIntensity * 0.05 * cos(uv.x * 6.0 + t);
                ...
                float n2 = snoise3(vec3(uv * 5.0 + 3.7, t * 0.7)) * 0.5 + 0.5;
